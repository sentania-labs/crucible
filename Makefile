...
proxy-config: ## write the egress proxy's allowlist from EGRESS_ALLOWLIST (13, S6)
	...
	@python3 -m crucible.application.proxy_config \
	  ...
	  $(if $(SPARK_ENDPOINT_URL),--local-endpoint "$(SPARK_ENDPOINT_URL)")
	@{ \
	...
	@echo "wrote var/egress/squid.conf with $(words $(EGRESS_ALLOWLIST)) remote hostname(s) and configured local model endpoints"
