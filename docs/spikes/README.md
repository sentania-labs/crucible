...
| S14 | Can Crucible observe subscription quota per harness, and wait and resume on it? | adopt with conditions: the pinned third-party reader answers for Codex and AGY from the dedicated credential directories only, with evidence it never touched the operator's own, and costs no provider quota; Claude Code's `setup-token` credential yields no reading at all (403), so the signal stays advisory and never blocks a launch ([S14.md](S14.md), ADR 0014 proposed) |
