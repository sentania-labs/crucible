...
| S12 | What does the external reviewer emit on a PR, under which login, and how is it triggered? | adjust design: with the repository set to review all PRs, an App-authored PR is reviewed automatically in 101 s under `chatgpt-codex-connector[bot]`; a clean review is only an `eyes` then `+1` reaction on the PR, which the App's grant cannot read (403), so Issues read is required and the provider setting is a repository-onboarding prerequisite ([S12.md](S12.md)) |
