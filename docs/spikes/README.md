...
| S14 | Can Crucible observe subscription quota per harness, and wait and resume on it? | ADR 0014 rejected 2026-09-19, reactive detection retained (see the ADR's Outcome); the spike's own finding was: adjust design, proceed with conditions: a pinned third-party reader answers for Codex and AGY from the dedicated credential directories only, proven by decoy runs not to fall back to a home store, and costs no model quota, though each Codex reading is one provider request; Claude Code yields nothing (its dedicated directory has no `.credentials.json`, and its setup-token gets 403), so the signal is advisory only, the quota deferral cannot reuse the concurrency-cap path that holds a checkout lease, and AGY's reading needs a harness CLI in the service container ([S14.md](S14.md), ADR 0014 proposed) |
