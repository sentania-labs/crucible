...
| S4 | With harness sandboxes disabled, does the container hardening hold? | proceed: every probe failed on the rootful daemon; re-run on rootless in C3 ([S4.md](S4.md)) |
...
| S6 | Which egress endpoints does each harness need? | adjust design: Claude Code allowlist confirmed live; Codex and AGY provisional pending an authenticated run; Codex gains `--disable plugins` ([S6.md](S6.md)) |
